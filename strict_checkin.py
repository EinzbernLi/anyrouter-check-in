...
SUMMARY_BALANCE_RE = re.compile(r'余额:\s*\$(-?\d+(?:\.\d+)?)\s*\|\s*累计消耗:\s*\$(-?\d+(?:\.\d+)?)')
...
def classify_anyrouter(obs: Observation) -> Verdict:
	...
	if obs.explicit_already_checked:
		return Verdict(
			obs.name, obs.provider, 'ALREADY_CHECKED', 'server explicitly reported an existing check-in', total, delta
		)
	if not obs.endpoint_success:
		return Verdict(
			obs.name, obs.provider, 'UNCONFIRMED', 'no confirmed /api/user/sign_in success was observed', total, delta
		)
	if delta is not None and delta > EPSILON:
		return Verdict(
			...
			obs.provider,
			'REWARDED',
			f'/api/user/sign_in succeeded and total quota increased by ${delta:.2f}',
			total,
			delta,
		)
	if delta is not None and abs(delta) <= EPSILON:
		return Verdict(
			...
			obs.provider,
			'ALREADY_CHECKED',
			'/api/user/sign_in succeeded with no quota increase',
			total,
			delta,
		)
	return Verdict(
		...
		obs.provider,
		'UNCONFIRMED',
		'/api/user/sign_in succeeded but before/after quota could not be verified',
		total,
		delta,
	)
...
def classify_agentrouter(obs: Observation, previous: dict | None, now: datetime) -> Verdict:
	...
	if total is None:
		return Verdict(
			obs.name, obs.provider, 'UNCONFIRMED', 'fresh login succeeded but quota could not be read', total
		)
...
def classify(observations: list[Observation], state: dict, now: datetime) -> list[Verdict]:
	...
	for obs in observations:
		...
		if provider == 'anyrouter':
			...
			verdict = Verdict(
				obs.name, provider, 'UNCONFIRMED', 'unsupported provider in strict verifier', obs.current_total()
			)
...
def build_summary(verdicts: list[Verdict], child_code: int) -> str:
	...
	lines.append(
		'[STRICT RESULT] REWARDED/ALREADY_CHECKED are confirmed; BASELINE_ESTABLISHED is operationally healthy but not a reward claim.'
	)
	return '\n'.join(lines)
